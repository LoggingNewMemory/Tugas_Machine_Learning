...
if __name__ == '__main__':
    ...
    # Try to ping first in case it's already running
    qwen_running = False
    try:
    ...
    if not qwen_running:
        ...
        if not os.path.exists(vllm_python):
            ...
            # Open log file to store Qwen output
    ...
    try:
        ngrok.set_auth_token("3EoikyeuSLFCb704gJbsrMQFD12_5Uo3aSMn8LAXSg72q7rqN")
        public_url = ngrok.connect(5000).public_url
        print("\n" + "="*80)
        print(f"🚀 WEB APP IS LIVE AND ACCESSIBLE FROM ANYWHERE AT: {public_url}")
        print("="*80 + "\n")
    except Exception as e:
